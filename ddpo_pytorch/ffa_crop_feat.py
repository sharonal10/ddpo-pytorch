...
class FFACropFeat(torch.nn.Module):
    def __init__(self, device, carvekit_object_type="object"):
        ...
        self.seg = models.segmentation.deeplabv3_resnet101(pretrained=True).cuda()
        self.seg.eval()
    ...
    def forward_single(self, x_list):
        ...
        with torch.no_grad():
            ...
            img_list = [img.resize((336, 336)) for img in x_list]
            img_list = [F.to_tensor(img).unsqueeze(0).cuda() for img in img_list]
            img_list = [self.seg(img)['out'][0].argmax(0) for img in img_list]
            # print(img_list[0])
            # print('aaa', np.unique(img_list[0].cpu().numpy()))
            img_list = [to_pil_image(tensor.float()) for tensor in img_list]
            preprocessed_imgs = self.preprocess(img_list)
...
# image0 = Image.open( rf'C:\Users\19255\Downloads\fruit-color\test.png')
# image_path1 = rf'C:\Users\19255\Downloads\fruit-color\strawberry-red.jpg'
# image1 = Image.open(image_path1)

# print(fm([image0], [image1]))
...
# print(fm([image1], [image3]))
# print(fm([image2], [image3]))